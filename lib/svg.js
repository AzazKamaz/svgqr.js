...
function generateSvg({mat, size = mat.length}, options) {
    const padding = 2 * (options && options.padding || 0);
    return `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="${0 - padding} ${0 - padding} ${(size + padding) * 2} ${(size + padding) * 2}"><path d="${generateSvgPath({mat}, options)}" fill-rule="evenodd"/></svg>`
}
